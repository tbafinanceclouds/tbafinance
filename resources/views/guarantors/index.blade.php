...
<div class="container mx-auto px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded shadow p-4">
            <p class="text-sm text-gray-500">Total Guarantors</p>
            <p class="text-2xl font-bold">{{ $guarantors->total() }}</p>
        </div>
        <div class="bg-white rounded shadow p-4">
            <p class="text-sm text-gray-500">Pending</p>
            <p class="text-2xl font-bold text-yellow-600">
                {{ $guarantors->where('status', 'pending')->count() }}
            </p>
        </div>
        <div class="bg-white rounded shadow p-4">
            <p class="text-sm text-gray-500">Approved</p>
            <p class="text-2xl font-bold text-green-600">
                {{ $guarantors->where('status', 'approved')->count() }}
            </p>
        </div>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4">{{ $guarantor->member->full_name ?? 'N/A' }}</td>
                    <td class="px-6 py-4">{{ $guarantor->relationship ?? 'N/A' }}</td>
                    <td class="px-6 py-4">UGX {{ number_format($guarantor->amount_guaranteed ?? 0, 2) }}</td>
                    <td class="px-6 py-4">
                        ...
                        @if(($guarantor->status ?? 'pending') == 'pending')
                            <form action="{{ route('guarantors.approve', $guarantor) }}" method="POST" class="inline">
                                ...
                                <button type="submit" class="text-green-500 hover:text-green-700 mr-2"
                                        onclick="return confirm('Approve this guarantor?')">Approve</button>
                            </form>
                            <form action="{{ route('guarantors.reject', $guarantor) }}" method="POST" class="inline">
                                ...
                                <button type="submit" class="text-red-500 hover:text-red-700 mr-2"
                                        onclick="return confirm('Reject this guarantor?')">Reject</button>
                            </form>
                        @endif
                ...
                <tr>
                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                        No guarantors found. 
                        <a href="{{ route('guarantors.create') }}" class="text-blue-500 hover:underline">
